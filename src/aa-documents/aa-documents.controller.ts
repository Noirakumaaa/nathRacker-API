...
export class AaDocumentsController {
  ...
  async importDocuments(
    ...
    return this.aaDocumentsService.importFromXlsx(code, file.buffer, sheet);
  }
  ...
  @Roles(Role.ENCODER, Role.ADMIN)
  @UseInterceptors(FileInterceptor('file', { storage: memoryStorage() }))
  async importWorkbook(@UploadedFile() file: Express.Multer.File) {
    if (!file) throw new BadRequestException('No file uploaded.');
    return this.aaDocumentsService.importWorkbook(file.buffer);
  }
  ...
  @Put('aa-documents/:id/monthly')
  @Roles(Role.ENCODER, Role.ADMIN)
  upsertMonthly(@Param('id') id: string, @Body() dto: UpsertAaMonthlyDto) {
    return this.aaDocumentsService.upsertMonthly(id, dto);
  }

  /** GET /aa-documents/:id/load26-monthly */
  @Get('aa-documents/:id/load26-monthly')
  getLoad26Monthly(@Param('id') id: string) {
    return this.aaDocumentsService.getLoad26Monthly(id);
  }

  /** PUT /aa-documents/:id/load26-monthly */
  @Put('aa-documents/:id/load26-monthly')
  @Roles(Role.ENCODER, Role.ADMIN)
  upsertLoad26Monthly(
    @Param('id') id: string,
    @Body() dto: UpsertAaLoad26MonthlyDto,
  ) {
    return this.aaDocumentsService.upsertLoad26Monthly(id, dto);
  }
}
