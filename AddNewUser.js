...
async function addNewUsers() {
    ...
    try {
        for (const user of newUsers) {
            ...
            if (existing) {
                console.warn(`Skipped (already exists): ${user.govUsername}`);
                continue;
            }
...
async function addOperationsOffices() {
    ...
    try {
        for (const office of newOperationsOffices) {
            ...
            if (existing) {
                console.warn(`Skipped (already exists): ${office.name}`);
                continue;
            }
...
async function addLgus() {
    ...
    try {
        for (const lgu of newLgus) {
            ...
            if (existing) {
                console.warn(`Skipped (already exists): ${lgu.name}`);
                continue;
            }
...
async function addBarangays() {
    ...
    try {
        for (const barangay of newBarangays) {
            ...
            if (existing) {
                console.warn(`Skipped (already exists): ${barangay.name}`);
                continue;
            }
...
async function verifyData() {
    ...
    try {
        ...
        for (const b of barangays) {
            console.log(`Barangay: ${b.name} (id:${b.id}) → LGU: ${b.lgu.name} (id:${b.lgu.id}) → Office: ${b.lgu.operationsOfficeNum.name} (id:${b.lgu.operationsOfficeNum.id})`);
        }
    }
    catch (error) {
        console.error('Error:', error);
